...
export async function onRequest(context) {
  ...
  if (!response.ok || data?.errorCode) {
    return fail(data?.errorDescription ?? "Realtime tracks/new failed", data, response.status || 500);
  }

  const failedTrack = data?.tracks?.find?.((track) => track?.errorCode);
  if (failedTrack) {
    return fail(failedTrack.errorDescription ?? "Realtime track failed", data, 502);
  }
